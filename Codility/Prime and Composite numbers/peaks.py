...
# More precisely, it is an index P such that 0 < P < N − 1,  A[P − 1] < A[P] 
# and A[P] > A[P + 1].
...
# of the blocks (for example A[K − 1] or A[K]) can also be peaks, but only if they have both 
...
#  particular that the first block (1, 2, 3, 4) has a peak at A[3], because A[2] < A[3] > A[4], 
...
def peaks(A):
    ...
    #print(facts)
    found = 0
    while len(facts) > 0 or found == 1: 
        start = 0
        end = []
        for i in range( facts[-1]):
            end.append(i*facts[-1])
        ...
        for i in range(facts[-1]):
            ...
            print(peaks[i: end])
        break
        # part = int(length/facts[-1])
        # for i in range(start,):
        #     allParts = []
        #     allParts.append([A[start:part]])
        #     start= part+1
        #     part = part + part
        #     print(allParts)
        # break
            # if 1 not in peaks[start:part]:
            #     facts.pop()
            # A[start:part]
            # print(A[start:part])
            # break
